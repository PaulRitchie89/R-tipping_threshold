...
Script that produces Figs 3, 7, 8, 9, 10, 11 in "Evaluating the skill of a
...
for j in range(len(t)-1):
    ...
    # Plot ROC curves at specified time intervals (Fig. 8)
    if t[j] in times:
...
ax3[0,1].legend(frameon=False,ncol=4,loc='lower center', bbox_to_anchor=(-0.1, 1.1))
...
# Plot Fig. 9
mat3_contents = sp.loadmat('AUC_bootstrap_wl'+str(wl)+'_'+str(H1).replace('.', 'p')+'_tstart'+str(tstart)+'_tup'+str(tup)+'_tpause'+str(tpause)+'_tdown'+str(tdown)+'.mat')

SN_AUC_boot = mat3_contents['SN_AUC_boot']
ST_AUC_boot = mat3_contents['ST_AUC_boot']
Edge_AUC_boot = mat3_contents['Edge_AUC_boot']
EWS_AUC_boot = mat3_contents['EWS_AUC_boot']

SN_AUC_low = np.quantile(SN_AUC_boot,0.025,axis=1)
SN_AUC_high = np.quantile(SN_AUC_boot,0.975,axis=1)
ST_AUC_low = np.quantile(ST_AUC_boot,0.025,axis=1)
ST_AUC_high = np.quantile(ST_AUC_boot,0.975,axis=1)
Edge_AUC_low = np.quantile(Edge_AUC_boot,0.025,axis=1)
Edge_AUC_high = np.quantile(Edge_AUC_boot,0.975,axis=1)
EWS_AUC_low = np.quantile(EWS_AUC_boot,0.025,axis=1)
EWS_AUC_high = np.quantile(EWS_AUC_boot,0.975,axis=1)

fig4,ax4 = plt.subplots(3,1,sharex=True)
ax4[0].plot(t[:-1],Forcing_profile,'k')
ax4[1].plot(t[:-1],SN_AUC,label='$S_N$')
ax4[1].fill_between(t[:-1],SN_AUC_low,SN_AUC_high,alpha=0.2)
ax4[1].plot(t[:-1],ST_AUC,label='$S_T$')
ax4[1].fill_between(t[:-1],1-ST_AUC_low,1-ST_AUC_high,alpha=0.2)
ax4[1].plot(t[:-1],Edge_AUC,label='R-tipping indicator')
ax4[1].fill_between(t[:-1],Edge_AUC_low,Edge_AUC_high,alpha=0.2)
ax4[1].plot(t[:-1],EWS_AUC,label=r'$\alpha^2$')
ax4[1].fill_between(t[:-1],EWS_AUC_low,EWS_AUC_high,alpha=0.2)
...
ax2[0].plot(t[:-1],SN_Informedness)
ax2[0].plot(t[:-1],ST_Informedness)
ax2[0].plot(t[:-1],Edge_Informedness)
ax2[0].plot(t[:-1],EWS_Informedness)
ax2[0].set_xlim(0,1000)
ax2[0].set_ylabel('Informedness')
ax2[0].set_title('Optimal threshold')
...
ax2[1].plot(t[:-1],SN_Informedness,label='$S_N$')
ax2[1].plot(t[:-1],ST_Informedness,label='$S_T$')
ax2[1].plot(t[:-1],Edge_Informedness,label='R-tipping indicator')
ax2[1].plot(t[:-1],EWS_Informedness,label='$\\alpha^2$')
ax2[1].set_ylabel('Informedness')
ax2[1].set_xlabel('Time (years)')
ax2[1].set_title('Fixed threshold')
sns.despine()
ax2[1].legend(frameon=False,ncol=4,loc='lower center', bbox_to_anchor=(0.5, -0.5))
...
fig5.subplots_adjust(top=0.9,bottom=0.18,left=0.115,right=0.97,hspace=0.122,wspace=0.2)
ax5[0].legend(frameon=False,ncol=3,loc='lower center', bbox_to_anchor=(0.5, 1))
ax5[1].legend(frameon=False,ncol=3,loc='upper center', bbox_to_anchor=(0.5, -0.2))
